...
class WhitelistAdd(BaseModel):
    prefix: str = Field(..., min_length=4, max_length=255,
                         description="Registry prefix, vd: 'docker.io/myorg/'. KẾT THÚC bằng '/' để tránh prefix collision.")
...
def _validate_prefix(raw: str) -> str:
    """Validate prefix in endpoint handler (KHÔNG dùng field_validator để tránh
    main.py's validation_exception_handler crash khi serialize ValueError).

    Returns normalized prefix (lowercase, ending '/').
    Raises HTTPException(422) với hint Vietnamese rõ ràng.
    """
    v = (raw or "").strip().lower()
    if "/" not in v:
        raise HTTPException(status_code=422, detail=(
            "prefix phải chứa '/' (vd: 'docker.io/myname/' hoặc "
            "'us-central1-docker.pkg.dev/myproject/myrepo/')"
        ))
    if not v.endswith("/"):
        v += "/"
    if v.startswith("*") or v.startswith("/"):
        raise HTTPException(status_code=422, detail="prefix không được bắt đầu bằng '*' hoặc '/'")
    ...
    for bad, hint in unsupported_registries.items():
        if v.startswith(bad):
            raise HTTPException(status_code=422, detail=f"Registry '{bad}' không support: {hint}")
    if v in ("docker.io/",):
        raise HTTPException(status_code=422, detail=(
...
async def add_whitelist(
    ...
    # Validate prefix in endpoint handler (raises HTTPException 422, not ValueError)
    normalized_prefix = _validate_prefix(body.prefix)

    # Check duplicate
    existing = (await db.execute(text(
        "SELECT id FROM workspace_image_whitelist WHERE workspace_id = :ws AND prefix = :p"
    ), {"ws": workspace_id, "p": normalized_prefix})).first()
    if existing:
        raise HTTPException(status_code=409, detail=f"Prefix '{normalized_prefix}' đã có trong whitelist")
    ...
    row = (await db.execute(text(
        ...
        "ws": workspace_id, "p": normalized_prefix, "desc": body.description,
        "sid": body.pull_secret_id, "by": str(me.id),
    })).mappings().first()
    await db.commit()

    log.info("[whitelist] ws=%s added prefix=%s by=%s", workspace_id, normalized_prefix, me.email)
